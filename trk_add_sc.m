function [header,tracks] = trk_add_sc(header,tracks,source,name)
...
%    source - Scalar MRI volume [3d array]. Alternatively, a single scalar to 
%             attach to all vertices.
...
for iTrk=1:length(tracks)
    if length(source) == 1
        % Assign a whole-tract property as a vertex-wise scalar
        scalars = repmat(tracks(iTrk).props(source), size(tracks(iTrk).matrix, 1), 1);
    else
        ...
        % Find vertices that don't map into the volume
        mask = ~logical(sum(vox < 1 | bsxfun(@minus, vox, header.dim) > 0, 2));
        
        % Index into volume to extract scalar values
        inds          = sub2ind(header.dim, vox(mask,1), vox(mask,2), vox(mask,3));
        scalars       = repmat(NaN, size(tracks(iTrk).matrix, 1), 1);
        scalars(mask) = source(inds);
